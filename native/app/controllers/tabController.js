...
TabController.init = async function(tabItem, cartModalController) {
    ...
    navigationView.on('cart:count-updated', (data) => {
        ...
        // We don't want to force the cart modal to update
        // unless the user has actually added or removed from cart
        if (data.userTriggered) {
            AppEvents.trigger(AppEvents.cartModalNeedsUpdate)
        }
    })
