...
const AccountTabController = function(viewPlugin, headerController, layout, segmentedView, signInView, registerView) {
    ...
    Astro.registerRpcMethod(AppRpc.names.loggedIn, [], () => {
        ...
        AppEvents.trigger(AppEvents.broadcastDidLogin)
    })
    ...
    Astro.registerRpcMethod(AppRpc.names.guest, [], () => {
        ...
        this.isLoggedIn = false
        AppEvents.trigger(AppEvents.broadcastDidLogout)
    })
}
...
AccountTabController.init = async function(cartModalController) {
    ...
    this.pwaBroadcasterSignInView = new PWABroadcaster(signInView)
    this.pwaBroadcasterRegisterView = new PWABroadcaster(registerView)
    ...
    signInView.on('open:cart-modal', () => {
        headerController.showCartModal()
    })

    registerView.on('open:cart-modal', () => {
        headerController.showCartModal()
    })
